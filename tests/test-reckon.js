...
// the act advances through beats over time, then resolves to an ending
const cad=GG.CONFIG.reckoningStageSec;
let beats=0, ticks=0;
while(!s.ending && ticks<20){ const n0=s.chronicle.length; Game.tick(s, cad); if(s.chronicle.length>n0) beats++; ticks++; }
ok(s.ending!==null, 'the Reckoning resolves to an ending after its beats');
ok(s.endgame.active===false, 'endgame deactivates once resolved');
ok(GG.ENDINGS[s.ending.id], 'ending id is a valid destiny');
ok(beats>=2, 'multiple Reckoning beats played before the end ('+beats+')');
